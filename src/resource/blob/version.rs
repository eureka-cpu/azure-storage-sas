...
use crate::{
    blob::{
        BLOB_DEFAULT_VERSION, BlobResourceOptions, BlobSasPermissions, BlobStringToSign,
        format_blob_time,
    },
    error::SasError,
...
mod tests {
    ...
    fn test_blob_version_string_to_sign_has_24_fields() {
        ...
        assert_eq!(
            parts.len(),
            26,
            "blob version string-to-sign must have 26 fields"
        );
        assert_eq!(parts[16], "bv", "[16] signedResource");
        assert_eq!(
            parts[17], "",
            "[17] signedSnapshotTime — empty for versions"
        );
    }
